...
public class CartesianGoalToCartesianPositionMapper
	...
	public ActuatorDriverMapperResult map(SoftRobotRuntime runtime, final CartesianActuatorDriver driver,
		...
		DoubleConditional overrideScaleFactor = fragment.add(new DoubleConditional(0.0, 1.0));
		fragment.connect(ports.cancel, overrideScaleFactor.getInCondition(), new StateDataflow());
		DoubleAverage overrideScale = fragment.add(new DoubleAverage(cancelDuration));
		fragment.connect(overrideScaleFactor.getOutValue(), overrideScale.getInValue());

		DoubleMultiply scaledOverride = fragment.add(new DoubleMultiply());
		fragment.connect(overridePort, scaledOverride.getInFirst(), new DoubleDataflow());
		fragment.connect(overrideScale.getOutValue(), scaledOverride.getInSecond());
		overridePort = fragment.addOutPort(new DoubleDataflow(), false, scaledOverride.getOutValue());

		DoubleEquals isStopped = fragment.add(new DoubleEquals(0.0, 0.0, 0.01));
		fragment.connect(overrideScale.getOutValue(), isStopped.getInFirst());
		...
		DataflowOutPort velocity = fragment.addOutPort(relDataflow, false, otg.getOutPos());

		final ActuatorDriverMapperResult result = runtime.getMapperRegistry().mapActuatorDriver(runtime, driver,
				new CartesianPositionActionResult(velocity), parameters, ports.cancel, ports.override);
		...
		BooleanOr doneOrCancelled = fragment.add(new BooleanOr(false, false));
		fragment.connect(done.getOutValue(), doneOrCancelled.getInFirst());
		fragment.connect(isStopped.getOutValue(), doneOrCancelled.getInSecond());

		final DataflowOutPort completed = fragment.addOutPort(new StateDataflow(), false,
				doneOrCancelled.getOutValue());
