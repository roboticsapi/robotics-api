...
public class PlatformWheelVelocityDoubleSensorMapper
		implements SensorMapper<SoftRobotRuntime, Double, PlatformWheelVelocityDoubleSensor> {

	@Override
	public SensorMapperResult<Double> map(final SoftRobotRuntime runtime,
			final PlatformWheelVelocityDoubleSensor sensor, SensorMappingPorts ports, SensorMappingContext context)
			throws MappingException {

		NetFragment fragment = new NetFragment("Platform Wheel Velocity Sensor");
		WheelMonitor monitor = fragment
				.add(new WheelMonitor(sensor.getDriver().getDeviceName(), sensor.getWheelNumber()));

		DataflowOutPort sensorPort = fragment.addOutPort(new DoubleDataflow(), false, monitor.getOutVel());
