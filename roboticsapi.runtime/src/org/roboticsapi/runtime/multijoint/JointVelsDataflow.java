...
/**
 * A data flow transmitting a set of joint velocity values
 */
public class JointVelsDataflow extends DataflowType {
	...
	public boolean providesValueFor(final DataflowType other) {
		...
		if (other instanceof JointVelsDataflow) {
			final JointVelsDataflow o = (JointVelsDataflow) other;
			if (o.count != count) {
				return false;
			}
	 ...
	 * @param multiJoindDeviceDriver device for this dataflow
	 */
	public JointVelsDataflow(final int count, final MultiJointDeviceDriver multiJoindDeviceDriver) {
		super(count);
		this.multiJointDeviceDriver = multiJoindDeviceDriver;
		this.count = count;
	}
	...
	public ValueReader[] createValueReaders(String uniqueKeyPrefix, List<OutPort> valuePorts, NetFragment fragment)
		...
		for (int i = 0; i < getCount(); i++) {
			ReadDoubleFromNet reader = fragment.add(new ReadDoubleFromNet(uniqueKeyPrefix + "_JointVelsDataflow_" + i));
