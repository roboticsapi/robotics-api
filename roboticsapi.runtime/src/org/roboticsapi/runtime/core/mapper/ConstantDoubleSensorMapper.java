...
public class ConstantDoubleSensorMapper implements SensorMapper<SoftRobotRuntime, Double, ConstantDoubleSensor> {

	@Override
	public SensorMapperResult<Double> map(SoftRobotRuntime runtime, ConstantDoubleSensor sensor,
			SensorMappingPorts ports, SensorMappingContext context) throws MappingException {

		NetFragment result = new NetFragment("Constant double");
		NetFragment val = new NetFragment("Value");
		DoubleValue value = val.add(new DoubleValue(sensor.getValue()));
		result.add(val);
		DataflowOutPort resultPort = val.addOutPort(new DoubleDataflow(), false, value.getOutValue());
