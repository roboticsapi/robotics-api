...
class SoftRobotWaitCommand extends WaitCommand implements MappedCommand {
	private CommandMapperResult result = null;
	private final SoftRobotCommandPorts ports = new SoftRobotCommandPorts();

	SoftRobotWaitCommand(String name, RoboticsRuntime runtime) {
		super(name, runtime);
	}

	SoftRobotWaitCommand(String name, RoboticsRuntime runtime, double duration) {
