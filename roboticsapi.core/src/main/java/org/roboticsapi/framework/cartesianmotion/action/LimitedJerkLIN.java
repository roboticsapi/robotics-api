...
import org.roboticsapi.framework.cartesianmotion.plan.LimitedJerkLINPlan;
...
public class LimitedJerkLIN extends PathMotion<LINPlan> {
	...
	public LimitedJerkLIN(double speedFactor, final Frame from, final Frame to) {
		this(1, new Pose(from), new Pose(to));
	}

	public LimitedJerkLIN(double speedFactor, final Pose from, final Pose to) {
		super(0);
	...
	public void calculatePlan(Map<PlannedAction<?>, Plan> plans, DeviceParameterBag parameters)
		...
		fromTransformation = getFrom().getTransformation();
		toTransformation = base
				.getTransformationTo(getTo().getReference(), World.getCommandedTopology().withoutDynamic())
