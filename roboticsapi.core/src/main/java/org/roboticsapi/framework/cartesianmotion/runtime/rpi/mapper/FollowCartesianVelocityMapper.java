...
public class FollowCartesianVelocityMapper implements ActionMapper<FollowCartesianVelocity> {

	@Override
	public ActionResult map(FollowCartesianVelocity action, DeviceParameterBag parameters, MapperRegistry registry,
			RealtimeBoolean cancel, RealtimeDouble override, RealtimeDouble time, Map<PlannedAction<?>, Plan> plans)
			throws MappingException, RpiException {
		RealtimeDouble zeroIfCancel = RealtimeDouble.createConditional(cancel, RealtimeDouble.ZERO, RealtimeDouble.ONE);
		RealtimeTwist twist = action.getVelocity().getTwist();
		RealtimeTwist scaledTwist = RealtimeTwist.createFromLinearAngular(
				twist.getTranslationVelocity().scale(override.multiply(zeroIfCancel)),
				twist.getRotationVelocity().scale(override.multiply(zeroIfCancel)));

		CartesianVelocityGoalActionResult ret = new CartesianVelocityGoalActionResult(action, cancel,
