...
public class CartesianPositionToJointPositionMapper<DD extends NamedActuatorDriver<? extends RobotArm>>
	...
	public RealtimeValueConsumerFragment map(DD actuatorDriver, CartesianPositionActionResult actionResult,
		...
		try {
			RealtimeDouble[] jointGoal = actuatorDriver.getDevice().getInverseKinematicsSensor(
					actionResult.getPosition().getTransformationForRepresentation(
							actuatorDriver.getDevice().getReferenceFrame(), topology),
					actuatorDriver.getDevice().getJointSensors(), parameters);

			return registry.mapDriver(actuatorDriver,
					new MultiJointActionResult(null, null, Stream.of(jointGoal)
