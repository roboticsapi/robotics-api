...
public class NullspacePTPMapper implements ActionMapper<NullspacePTP> {

	@Override
	public ActionResult map(final NullspacePTP action, DeviceParameterBag parameters, MapperRegistry registry,
		...
		PTP ptp = new PTP(action.getFrom(), action.getTo());
		ActionResult joints = registry.mapAction(ptp, parameters, cancel, override, time, plans);
		Plan firstPlan = plans.get(ptp);

		ptp = new PTP(firstPlan.getTotalTime() / duration, action.getFrom(), action.getTo());
