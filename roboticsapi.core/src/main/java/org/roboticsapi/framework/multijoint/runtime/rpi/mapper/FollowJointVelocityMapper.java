...
public class FollowJointVelocityMapper implements ActionMapper<FollowJointVelocity> {

	@Override
	public ActionResult map(FollowJointVelocity action, DeviceParameterBag parameters, MapperRegistry registry,
		...
		MultiJointActionResult ret = new MultiJointActionResult(action, cancel,
				Stream.of(action.getVelocity())
						.map(jv -> new JointVelocityGoalActionResult(null, null, jv.multiply(override)))
