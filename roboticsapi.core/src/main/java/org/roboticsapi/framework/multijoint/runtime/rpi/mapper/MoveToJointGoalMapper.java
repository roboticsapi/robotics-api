...
public class MoveToJointGoalMapper implements ActionMapper<MoveToJointGoal> {

	@Override
	public ActionResult map(MoveToJointGoal action, DeviceParameterBag parameters, MapperRegistry registry,
		...
		MultiJointActionResult ret = new MultiJointActionResult(action, RealtimeBoolean.TRUE,
				Stream.of(action.getGoal()).map(pos -> new JointPositionGoalActionResult(null, null, pos))
