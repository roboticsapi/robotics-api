...
import org.roboticsapi.framework.platform.MecanumPlatform;
...
public class MecanumPlatformGenericDriver extends AbstractActuatorDriver<MecanumPlatform>
	...
	private final Map<Integer, Relation> measuredWheelRelations = new HashMap<Integer, Relation>();
	...
	protected void onPresent() {
		...
		for (int i = 0; i < getDevice().getWheels().length; i++) {
			...
			measuredWheelRelations.put(i,
					createMeasuredWheelConnection(getDevice().getMountFrame(i), getDevice().getWheel(i).getBase(), i));
			measuredWheelRelations.get(i).establish();
		}
	...
	protected void onAbsent() {
		...
		for (int i = 0; i < getDevice().getWheels().length; i++) {
			...
			Relation measuredwheelRelation = measuredWheelRelations.remove(i);
			if (measuredwheelRelation != null) {
				measuredwheelRelation.remove();
			}
		}
	}
	...
	protected Relation createWheelConnection(Frame from, Frame to, int wheelNumber) {
		...
		return new CommandedPosition(from, to, sensor, velSensor);
	}
	 ...
	 * @return the created connection.
	 */
	protected Relation createMeasuredWheelConnection(Frame from, Frame to, int wheelNumber) {
		RealtimeDouble pSensor = getWheelPositionSensor(wheelNumber);
		RealtimeDouble vSensor = getWheelVelocitySensor(wheelNumber);
