...
public abstract class AbstractDifferentialWheeledPlatform extends AbstractPlatform<PlatformDriver>
	...
	protected final Connection createMountConnection(int wheelNumber) {
		switch (wheelNumber) {
		case LEFT_WHEEL:
			return new StaticConnection(new Transformation(getWheelOrientation(), getLeftWheelTranslation()));

		case RIGHT_WHEEL:
			return new StaticConnection(new Transformation(getWheelOrientation(), getRightWheelTranslation()));
		default:
			return null;
		}
	}
	...
	protected final Frame createMountFrame(int wheelNumber) {
		switch (wheelNumber) {
		case LEFT_WHEEL:
			return new Frame(getName() + " - Left mount frame");

		case RIGHT_WHEEL:
			return new Frame(getName() + " - Right mount frame");
