...
public final class RmiServer implements RoboticsObjectVisualizationServer {
	...
	public static FrameTopology chooseTopology() {
		String topology = System.getProperty("org.roboticsapi.remote.client.topology", "commanded");
		if (topology.equals("commanded")) {
			return World.getCommandedTopology();
		} else if (topology.equals("measured")) {
			return World.getMeasuredTopology();
		} else {
			RAPILogger.getLogger(RmiServer.class)
					.warning("Invalid visualization topology '" + topology + "', expected 'measured' or 'commanded'.");
			return World.getCommandedTopology();
		}
	}
	...
	private void onRemoteVisualizationAdded(RmiVisualizationClient factory) throws RemoteException {
		synchronized (visualizationGraphLock) {
			...
			VisualizationGraph visualization = new VisualizationGraph(scene, world.getOrigin(), chooseTopology());
