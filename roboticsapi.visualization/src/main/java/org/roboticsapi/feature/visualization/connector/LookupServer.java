...
//			lookupServer.close();
	...
	private void startAndManageServer(int port, final ConfigCache lastConfigurationFromOvertakenServer)
		...
		RAPILogger.getLogger(getClass()).fine("Started local lookup server on port " + port + ".");
		...
		try {
			while (alive) {
				try {
					...
					RAPILogger.getLogger(getClass()).fine("Incoming connection established from "
			...
			RAPILogger.getLogger(getClass()).fine("Closed local lookup server on port " + port + ".");
		}
	}
	...
	private RapiInfo[] bindToOtherServer(int port) {
		...
		try {
			...
			RAPILogger.getLogger(LookupServer.class).fine("Established connection to other lookup server "
			...
			try {
				...
				RAPILogger.getLogger(LookupServer.class).fine("Closed connection from other lookup server "
						+ clientSocket.getRemoteSocketAddress().toString() + ".");
				try {
					clientSocket.close();
				} catch (IOException e) {
